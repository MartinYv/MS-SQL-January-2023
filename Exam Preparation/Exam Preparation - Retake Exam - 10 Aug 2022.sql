...
-- 08 Tourist Sites established BC
  ...
  SELECT s.Name AS Site, l.Name AS Location,	l.Municipality,	l.Province, s.Establishment
	...
	LEFT JOIN Sites AS s
	  ON l.Id = s.LocationId
   WHERE s.Establishment LIKE '%[^0-9]BC%' AND SUBSTRING(l.Name,1,1) NOT IN ('B', 'M', 'D')
ORDER BY s.Name ASC
...
-- 10. Tourists visiting History & Archaeology sites
  ...
  SELECT DISTINCT SUBSTRING(t.Name, CHARINDEX(' ', t.Name)+1, LEN(t.Name)) AS LastName,	t.Nationality,	t.Age, t.PhoneNumber
    FROM Tourists AS t INNER JOIN SitesTourists AS st
...
UPDATE Tourists 
   SET Reward = CASE WHEN @counter >= 100 THEN 'Gold badge'
					 WHEN @counter	>= 50  THEN 'Silver badge'
			   		 WHEN @counter	>= 25  THEN 'Bronze badge'
			    END
